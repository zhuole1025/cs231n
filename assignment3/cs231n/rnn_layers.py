...
def rnn_backward(dh, cache):
    ...
    N, T, H = dh.shape
    dxi, dprev_h, dWx, dWh, db = rnn_step_backward(dh[:,T-1,:], cache[T-1])
    D = dxi.shape[1]
    dx = np.zeros((N, T, D))
    dx[:,T-1,:] = dxi
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cc_i ,cc_f, cc_o, cc_g = np.split(a, 4, axis=1)
    i = sigmoid(cc_i)
    f = sigmoid(cc_f)
    o = sigmoid(cc_o)
    g = np.tanh(cc_g)
    ...
    cache = (x, Wx, Wh, a, i, f, o, g, prev_c, prev_h, next_c)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    x, Wx, Wh, a, i, f, o, g, prev_c, prev_h, next_c = cache
    ...
    ifo = np.hstack((i, f, o))
    difo = np.hstack((di, df, do))

    dsigmoid =  difo * ifo * (1 - ifo)
    dtanh = dg * (1 - g**2)
    
    da = np.hstack((dsigmoid, dtanh))
    dWh = prev_h.T.dot(da)
    dWx = x.T.dot(da)
    db = np.sum(da, axis=0)
    dprev_h = da.dot(Wh.T)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
      if i == 0:
          h[:,i,:], next_c, cachei = lstm_step_forward(x[:,i,:], h0, next_c, Wx, Wh, b)
      else:
          h[:,i,:], next_c, cachei = lstm_step_forward(x[:,i,:], h[:,i-1,:], next_c, Wx, Wh, b)
      cache.append(cachei)
