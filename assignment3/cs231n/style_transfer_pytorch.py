...
def content_loss(content_weight, content_current, content_original):
    ...
    c = content_current.reshape((C_l, -1))
    o = content_original.reshape((C_l, -1))
    loss = torch.sum(torch.square(c - o)) * content_weight
...
def gram_matrix(features, normalize=True):
    ...
    flat = features.reshape((N, C, -1))
    for i in range(N):
        for j in range(C):
            for k in range(C):
                gram[i,j,k] = torch.sum(features[i,j,:,:] * features[i,k,:,:])
    if (normalize):
      gram /= H * W * C
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i in range(len(style_layers)):
      idx = style_layers[i]
      loss += torch.sum(torch.pow(gram_matrix(feats[idx]) - style_targets[i], 2)) * style_weights[i]
...
def extract_features(x, cnn):
    ...
    for i, module in enumerate(cnn._modules.values()):
        next_feat = module(prev_feat.type(torch.cuda.FloatTensor))
