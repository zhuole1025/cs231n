...
def generator_loss(logits_fake):
    ...
    return bce_loss(logits_fake, torch.ones(logits_fake.shape[0]).type(dtype))
...
def get_optimizer(model):
    ...
    optimizer = optim.Adam(model.parameters(), betas = (0.5, 0.999), lr = 1e-3)
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss = torch.mean(torch.pow(scores_real -1, 2))/2 + torch.mean(torch.pow(scores_fake, 2))/2
