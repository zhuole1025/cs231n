...
def layernorm_backward(dout, cache):
    ...
    dgamma = np.sum(dout * cache['z'], axis=0, keepdims=True)

    N = dout.shape[1]
    z = cache['z']
    dfdz = dout * cache['gamma']                                    #[NxD]
    dfdz_sum = np.sum(dfdz,axis=1, keepdims=True)                                  #[1xD]
    dx = dfdz - dfdz_sum/N - np.sum(dfdz * z,axis=1, keepdims=True) * z/N          #[NxD]
    dx /= cache['std']
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    out = np.zeros((N, F, int(outH), int(outW)))
    ...
    x_col = np.zeros((int(C*FH*FW), int(outH*outW)))                   #[C*FH*FW x H'*W']
    for index in range(N):
        ...
        for i in range(0, H_pad-FH+1, stride):
            for j in range(0, W_pad-FW+1,stride):
                ...
                neuron += 1
        out[index] = (w_row.dot(x_col) + b.reshape(F,1)).reshape(int(F), int(outH), int(outW))
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def max_pool_forward_naive(x, pool_param):
    ...
    maxpool = np.zeros((PH,PW))
    N, C, H, W = x.shape
    out = np.zeros((N, C, 1 + (H - PH) // stride, 1 + (W - PW) // stride))
    
    for i in range(N):
        for j in range (C):
          for m in range(1 + (H - PH) // stride):
            for n in range(1 + (W - PW) // stride):
